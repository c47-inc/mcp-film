...
The Cloudflare worker generated by the build redirects `www.mcp.film` to
`mcp.film` before serving assets, so humans and agents land on one canonical
host and analytics do not split across two domains.
